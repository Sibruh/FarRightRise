...
if not (path.exists(countryDir) and path.exists(partyDir)):
    ...
    content += ("}\n\n")
    content += ("String.prototype.convertToRGB = function(){\n")
    content += ("\tif(this.length != 6){\n")
    content += ("\t\tthrow \"Only six-digit hex colors are allowed.\";\n")
    content += ("\t}\n")
    content += ("\tvar aRgbHex = this.match(/.{1,2}/g);\n")
    content += ("\tvar aRgb = [parseInt(aRgbHex[0], 16), parseInt(aRgbHex[1], 16), parseInt(aRgbHex[2], 16)];\n")
    content += ("\treturn aRgb;\n")
    content += ("}\n\n")
